...
## Purpose
Capture, retrieve, or manage **quick, informal notes** stored in `.eca/notes.md` in the repository root.
**Case-insensitive**: `add`, `Add`, and `ADD` all work the same way.

## Commands
- **Add**: Save a new note. Format: `/note <Your note here>`
- **List**: View all notes. Format: `/note list`
- **Refer**: Retrieve a specific note. Format: `/note refer: <Note or ID>`
- **Delete**: Remove a note. Format: `/note delete: <Note or ID>`
...
3. **Output**:
   - For `add`: Confirm with "Note added."
   - For `list`: Return all notes in a clean format (no file paths).
   - For `refer`: Return the matched note or "Note not found."
   - For `delete`: Confirm with "Note deleted."
4. **Errors**:
   - If $ARGUMENTS is empty or invalid, respond:
     "Usage: /note <note> or /note <list/refer/delete>: <details>."

## Examples
- `/note Don't forget to refactor auth`
- `/note LIST`
